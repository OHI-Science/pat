...
```{r}
estatus_spp <- read_excel("C:/github/pat/prep/SPP/estatus.csv")
estatus<- filter(estatus, year >=2000)
```
...
estatus<- merge(estatus, trend)

trend_spp<- estatus %>% group_by(rgn_id) %>% summarise(trend = mean(trend))
